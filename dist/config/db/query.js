...
const convertPgToSqlite = (sql) => {
    ...
    converted = converted.replace(/datetime\('now'\)\s*-\s*INTERVAL\s*'(\d+)\s*(hours?|days?|minutes?)'/gi, (_match, num, unit) => `datetime('now', '-${num} ${unit}')`);
...
const query = async (text, params = []) => {
    try {
        ...
        const cleanParams = params.map(p => {
            ...
            if (p === undefined)
                return null;
            if (typeof p === 'object' && p !== null && !(p instanceof Buffer))
                return JSON.stringify(p);
            return p;
        });
        if (isReadOnlyQuery) {
            const stmt = pool_1.db.prepare(sqliteSQL);
            ...
            if (hasReturning) {
                ...
                const stmt = pool_1.db.prepare(sqlClean);
                ...
                if (table && result.changes > 0) {
                    ...
                    if (insertMatch && result.lastInsertRowid) {
                        followUp = pool_1.db.prepare(`SELECT * FROM ${table} WHERE id = ?`).all(result.lastInsertRowid);
                    }
                    else if (updateMatch) {
                        ...
                        if (whereMatch) {
                            ...
                            followUp = pool_1.db.prepare(`SELECT * FROM ${table} WHERE ${whereMatch[1]}`).all(...whereParams);
                        }
                        else {
                            followUp = [];
                        }
                ...
                const stmt = pool_1.db.prepare(trimmed);
                const result = stmt.run(...cleanParams);
                return { rows: [], rowCount: result.changes };
            }
        }
        ...
        const isReadOnlyQuery = /^SELECT\b/i.test(trimmed);
        (0, logger_1.logError)(`SQLite query error: ${err.message} | SQL: ${text}`);
        if (isReadOnlyQuery) {

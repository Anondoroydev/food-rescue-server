...
const initDB = async () => {
    ...
    `);
        (0, logger_1.logInfo)('Connected to SQLite database successfully.');
    }
    catch (error) {
        (0, logger_1.logError)(`SQLite initialization error: ${error.message}`);
        throw error;
    }
    // Seed admin user
    const adminCheck = await (0, query_1.query)(`SELECT id FROM users WHERE email = ?`, ['admin@foodrescue.com']);
    ...
    (0, logger_1.logInfo)('Database tables, indexes, and seed data initialized successfully.');
};
